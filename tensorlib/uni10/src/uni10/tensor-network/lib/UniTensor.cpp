...
UniTensor::UniTensor(const std::vector<Bond>& _bonds, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    initUniT(RTYPE);
  }
  catch(const std::exception& e){
...
UniTensor::UniTensor(const std::string stp, const std::vector<Bond>& _bonds, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    if(stp == "R")
      initUniT(RTYPE);
    else if(stp == "C")
...
UniTensor::UniTensor(const std::vector<Bond>& _bonds, std::vector<int>& _labels, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    initUniT(RTYPE);
    setLabel(_labels);
  }
...
UniTensor::UniTensor(const std::vector<Bond>& _bonds, int* _labels, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    initUniT(RTYPE);
    setLabel(_labels);
  }
