...
UniTensor::UniTensor(Real val): status(0), ongpu(false){ //GPU
  try{
    initUniT(RTYPE);
...
UniTensor::UniTensor(rflag _tp, const std::vector<Bond>& _bonds, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    throwTypeError(_tp);
    initUniT(_tp);
  }
...
UniTensor::UniTensor(rflag _tp, const std::vector<Bond>& _bonds, std::vector<int>& _labels, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    throwTypeError(_tp);
    initUniT(_tp);
    setLabel(_labels);
...
UniTensor::UniTensor(rflag _tp, const std::vector<Bond>& _bonds, int* _labels, const std::string& _name): name(_name), status(0), bonds(_bonds), ongpu(false){
  try{
    throwTypeError(_tp);
    initUniT(_tp);
    setLabel(_labels);
...
void UniTensor::initUniT(rflag tp){ //GPU
  ...
  // Need be modefied after adding GPU flag
  //TelemAlloc(RTYPE);
  TelemAllocForce(RTYPE);
  initBlocks(RTYPE);
  TelemBzero(RTYPE);
}
...
void UniTensor::TelemAllocForce(rflag tp){
  elem = (Real*)elemAllocForce(sizeof(Real) * m_elemNum, ongpu);
}
